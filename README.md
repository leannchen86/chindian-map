...
The DQL panel in the UI shows the intended Diffbot handoff: facet each cluster by Bay Area city, then compute pairwise correlations from raw count distributions.

The default UI is intentionally sparse: density is conveyed by a weather-map-style heat gradient, subtle contour lines, and a small legend. Exact counts, rankings, pairings, correlations, and DQL are tucked into the `i` details drawer or map popups.
